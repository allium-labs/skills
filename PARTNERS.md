...
Partner skills live alongside first-party skills under `skills/` with a vendor-prefixed directory name (e.g. `skills/alchemy-api/`). Each partner skill includes:
...
2. Copy the partner's skill directory under `skills/<vendor>-<skill-name>/` with their LICENSE.txt and `metadata.author` preserved.
