...
Retrieve DeFi positions for a wallet across multiple protocols and chains (Uniswap-v3, Aave, etc.).
...
| ---- | ---- | -------- | ----------- |
| `--chain` | string | Yes | Lowercase chain name (repeatable) |
| `--address` | string | Yes | Wallet address (repeatable; pair with `--chain`) |
| `--limit` | integer | No | Max positions returned (1–100). |
...
| ----- | ---- | -------------- | ----------- |
| `items[].chain` | string | Yes | Lowercase chain name |
| `items[].address` | string | Yes | Wallet address holding the position |
...
| `items[].position_type` | string | No | Position type |
...
| `items[].token0` | object | Yes | First token in the pool pair (full token metadata) |
| `items[].token1` | object | Yes | Second token in the pool pair (full token metadata) |
...
> Access: `items[]` for the result items. Paginate with `--cursor` from the response.

Detailed docs: `GET /api/v1/docs/docs/browse?path=api/developer/defi-positions/get-positions.md`
